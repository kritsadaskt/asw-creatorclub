...
const badgeMap: Record<string, { label: string; className: string }> = {
  ...
  asw_partners: {
    label: CreatorTypeNameByKey('asw_partners'),
    className:
      'inline-flex items-center shrink-0 px-1.5 py-0.5 rounded-md font-normal text-xs leading-none bg-yellow-50 text-yellow-700 border border-yellow-200/80',
  },
};
...
function inviteTypeToBadgeKey(raw: string): string | null {
  ...
  if (!t) return null;
  if (t === 'general' || t === 'resident') return null;
  ...
  if (t === 'asw_partners') return 'asw_partners';
